...
#other file
from data_extract_eu152_0703_14 import bin_no, energy_data, livetime_data, isotope
...
#vacuum.add_element('Al', 1, 'ao')
#vacuum.set_density('g/cm3', 2.7)

#europium = openmc.Material(name="europium152")
#europium.add_element('Al', 1, 'ao')
#europium.set_density('g/cm3', 2.7)

materials = openmc.Materials([al, be, ge, mylar, pb]) #, vacuum, europium])
...
leadshield_width = 1
leadshield_height = 9
sourcedistance = 1.4 + be_cap_cyl_pos + be_cap_cyl_height
...
#source_sphere = openmc.Sphere(r=0.2, x0=0, y0=0, z0=sourcedistance, boundary_type='transmission')
...
#source_sphere_region = -source_sphere
vacuum_void_region = -world_boundary & ~detector_region & ~al_case_i_region & ~al_case_o_region & ~almy_cap_region & ~almy_cap_2_region & ~be_cap_region & ~lead_shield_1 & ~lead_shield_2 #& ~source_sphere_region
...
#source_cell = openmc.Cell(region=source_sphere_region, fill=europium)
vacuum_void_cell = openmc.Cell(region=vacuum_void_region)

#universe
universe = openmc.universe = openmc.Universe(cells=[detector_cell, al_case_o_cell, al_case_i_cell, almy_cap_cell, almy_cap_2_cell, be_cap_cell, vacuum_void_cell, lead_shield_cell_1, lead_shield_cell_2])#, source_cell])
...
#plotxz.figure.savefig("eu152_0703_14/plots_eu152_0703_14"+"/xz-cell.png")
...
#plotxy.figure.savefig("eu152_0703_14/plots_eu152_0703_14"+"/xy-cell.png")
#plotyz = geometry.plot(pixels=(1000,1000), basis='yz', colors=color_assignment)
#plotyz.figure.savefig('yz-cell.png')

#Source
#eu-152 source energy profile
df = pandas.read_csv('misc/eu152_decay.csv', usecols=['energy','intensity'])
source_activity = 97654 * df.intensity.sum() * livetime_data #activity * decay% * collection time

source = openmc.IndependentSource()
source.particle = "photon"
source.space = openmc.stats.Point((0,0, sourcedistance)) #source location
source.energy = openmc.stats.Discrete(df.energy.to_numpy(), df.intensity.to_numpy()) #source energy profile
source.angle = openmc.stats.Isotropic()
source.strength = source_activity 
...
energy_bins = np.linspace(first_energy_bin,last_energy_bin,bin_no) #tally bins energy
...
#detector_tally_inc.scores = ['flux']
